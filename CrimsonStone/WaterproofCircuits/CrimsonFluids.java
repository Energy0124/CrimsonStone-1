...
public abstract class CrimsonFluids extends Block
{

    protected CrimsonFluids(int j, Material material)
    {
    ...
    public boolean b(IBlockAccess iblockaccess, int j, int k, int l)
    {
        return material != Material.LAVA;
    }

    public static float d(int j)
    {
        if(j >= 8)
            j = 0;
    ...
    protected int c(IBlockAccess iblockaccess, int j, int k, int l)
    {
    ...
    public int getDropType(int j, Random random, int k)
    {
        return 0;
    }
    ...
    public Vec3D d(IBlockAccess iblockaccess, int j, int k, int l)
    {
        Vec3D vec3d = Vec3D.create(0.0D, 0.0D, 0.0D);
        int i1 = c(iblockaccess, j, k, l);
        for(int j1 = 0; j1 < 4; j1++)
        {
            ...
            if(j1 == 3)
                i2++;
            int j2 = c(iblockaccess, k1, l1, i2);
            if(j2 < 0)
            {
                if(iblockaccess.getMaterial(k1, l1, i2).isSolid())
                    continue;
                j2 = c(iblockaccess, k1, l1 - 1, i2);
                if(j2 >= 0)
                {
                    int k2 = j2 - (i1 - 8);
    ...
    public void onPlace(World world, int j, int k, int l)
    {
        i(world, j, k, l);
    }
    ...
    public void i(World world, int j, int k, int l)
    {
    ...
    protected void fizz(World world, int j, int k, int l)
    {
